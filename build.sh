...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e FMDemodulator.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/FMDemodulator-1.0.0
        tar czf ${tmpdir}/FMDemodulator-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} FMDemodulator-1.0.0
        rpmbuild -ta ${tmpdir}/FMDemodulator-1.0.0.tar.gz
        rm -rf $tmpdir
    else
